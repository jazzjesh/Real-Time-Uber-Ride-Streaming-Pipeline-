...
dp.create_streaming_table(
    name="stg_rides",
    comment="Staging table for Uber rides - combines bulk historical and streaming data"
)

# =============================================
# BULK/INITIAL LOAD - Historical rides
# =============================================
@dp.append_flow(target="stg_rides")
def rides_bulk():
    """One-time bulk load from historical data"""
...
# =============================================
# STREAMING LOAD - Incremental rides from Event Hubs
# =============================================
@dp.append_flow(target="stg_rides")
def rides_stream():
    """Continuous streaming load from Event Hubs"""
    df = spark.readStream.table("rides_raw")
    
    df_parsed = (df.withColumn("parsed_rides", from_json(col("rides"), rides_schema))
                   .select("parsed_rides.*"))
    
    df_parsed = df_parsed.withColumn("booking_timestamp", 
                                     col("booking_timestamp").cast(TimestampType()))
    df_parsed = df_parsed.withColumn("updated_at", current_timestamp())
    return df_parsed
